...
module inst_cache(
    ...
    always_comb begin
        // decode the address
        logic [`INST_CACHE_TAG_WIDTH-1:0] tag = addr_i[`ADDR_WIDTH-1:`INST_CACHE_INDEX_WIDTH+2];
        logic [`INST_CACHE_INDEX_WIDTH-1:0] index = addr_i[`INST_CACHE_INDEX_WIDTH+1:2];
        
        // get the cache line
        logic [`INST_CACHELINE_WIDTH-1:0] cache_line = cache[index];

        // get 4 tags and valid bits
        logic valid0 = cache_line[224-1];
        logic [`INST_CACHE_TAG_WIDTH-1:0] tag0 = cache_line[223-1:200];
        logic valid1 = cache_line[168-1:0];
        logic [`INST_CACHE_TAG_WIDTH-1:0] tag1 = cache_line[167-1:144];
        logic valid2 = cache_line[112-1:0];
        logic [`INST_CACHE_TAG_WIDTH-1:0] tag2 = cache_line[111-1:88];
        logic valid3 = cache_line[56-1:0];
